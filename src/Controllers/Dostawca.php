...
class Dostawca extends Controller
{
  ...
  public function delete($id)
  {
    ...
    public function add($data = null)
    {
      if($_SESSION['role']<=1) // sprawdzenie czy zalogowany user ma prawa do modyfikacji konta pracownika
      {
        ...
        $view=$this->getView('Dostawca');   //utworzenie widoku
    ...
    public function insert()
    {
      if($_SESSION['role']<=1)
      {
            ...
            if($model)
            {
              ...
              $data = $model->insert($_POST['pelnanazwa'],$_POST['imie'],$_POST['nazwisko'],$_POST['Telefon1'],$_POST['Adres'],$_POST['KodPocztowy'],$_POST['Poczta'],$_POST['Email']);
              //pobranie komunikatów o bledach
            }
            if($data['error'] === "") // jeśli bledy nie istnieją, przechodzimy do zakladnki "Dostawca"
              {
                $this->redirect('Dostawca/');
              }
